...
class Parser {
	...
	private parseAgentDecl(): AgentDecl {
		...
		while (!this.check(TokenType.RBrace) && !this.check(TokenType.EOF)) {
			if (this.check(TokenType.Role)) {
				...
				// extension: 'api_configuration: "profile"' — selects the
				...
				// extension: 'peers: [@Agent1, @Agent2]' — declared direct-message peers.
				this.advance() // "peers"
				this.expect(TokenType.Colon)
				this.expect(TokenType.LBracket)
				meta.peers = []
	...
	private parseStakeOp(): StakeOp {
		...
		if (this.match(TokenType.Arrow)) {
			// Accept both Ident and AgentRef for recipients (extension)
		...
		// Optional per-stake `timeout(N)` / `retries(N)` clauses (extension).
	...
	private parseAwaitOp(): AwaitOp {
		...
		// Accept Ident, AgentRef, or Star for sources (extension: * is wildcard)
	...
	private parseSource(): Source {
		...
		// Accept bare Ident as a source ref (extension).
