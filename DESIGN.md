...
| `run_workflow` (synchronous) | ✅ | real model calls |
| `get_workflow_state` / `get_topology` | ✅ | unit |
...
- **Output-contract Layer 1 is `outputFormat`, not a strict tool.** In-process SDK MCP tools
  ...
  soft + retry.
- **`run_workflow` is synchronous** (runs to completion) for now; the background /
  `await_workflow_event` long-running model is not yet built. Escalation still works because
  elicitation happens *within* the open `run_workflow` call.
...
**Not yet implemented:** parallel stake dispatch, background execution + `await_workflow_event`,
`step_workflow` / `abort_workflow`, `get_sequence`, plugin packaging/persistence (Phase 5).
